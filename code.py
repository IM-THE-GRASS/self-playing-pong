	...
	def move_paddles(self):
		...
		if self.ball_x < reaction_dist:
			if random.random() < 0.6:
				...
				target_y = random.randint(0, HEIGHT - PADDLE_HEIGHT)
			self.left_y += int((target_y - self.left_y) * 0.3)
		else:
			if random.random() < 0.15:
				self.left_y += random.choice([-2, -1, 0, 1, 2])

	 
		# right paddle ( they r not the same to make it less boring )
